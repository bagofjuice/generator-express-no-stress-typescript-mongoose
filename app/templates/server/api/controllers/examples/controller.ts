...
export class Controller {
  ...
  async create(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      return res.status(HttpStatus.CREATED).location(`<%= apiRoot %>/examples/${doc._id}`).json(doc);
    }
    catch (err) {
      return next(err);
    }
  ...
  async patch(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      return res.status(HttpStatus.OK).location(`<%= apiRoot %>/examples/${doc._id}`).json(doc);
    }
    catch (err) {
      return next(err);
    }
